...
# Ширины колонок таблицы: чего нет в списке — по умолчанию. Тексту нужно
# место, координате и году — нет.
_WIDTHS = {"title": 42, "summary": 70, "quote": 70, "url": 45, "place_text": 34,
           "period_raw": 26, "work": 34, "actor": 26, "layer_title": 26, "uid": 24}


def _fill_sheet(ws, records: Iterable[ContextRecord], headers: Sequence[str]) -> int:
    """Пишет на лист шапку и записи, возвращает число записей.

    Один и тот же лист пишут обе выгрузки — на один слой и на все сразу;
    отличаются они только тем, сколько таких листов заводят.
    """
...
def _headers(russian: bool) -> list[str]:
    return [COLUMNS_RU[c] if russian else c for c in COLUMNS]
...
def write_xlsx(records: Sequence[ContextRecord], path: Path, *, sheet_name: str = "Данные",
    ...
    n = _fill_sheet(ws, records, _headers(russian_headers))
    wb.save(path)
    return n
...
def write_xlsx_multi(sheets: dict[str, Sequence[ContextRecord]], path: Path,
    ...
    wb = Workbook()
    wb.remove(wb.active)
    headers = _headers(russian_headers)
    counts = {name: _fill_sheet(wb.create_sheet(title=_safe_sheet(name, wb)), records, headers)
