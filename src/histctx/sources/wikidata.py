    ...
    for row in rows:
        ...
        if year_from and not year_to:
            if is_event:
                ...
                # Основанный позже 1960 года получает свой же год: иначе
                # интервал вывернется наизнанку (год от 1966, год до 1960),
                # и `overlaps_years` не найдёт объект даже в его собственном
                # году. Такие объекты в слоях есть — газовые месторождения и
                # советские заводы шестидесятых приходят вместе с прочими.
